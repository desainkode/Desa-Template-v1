...
const PotensiDesa = () => {
  ...
  return (
    <section className="bg-blue relative bg-center bg-no-repeat bg-contain">
      <div className="container mx-auto lg:max-w-xl md:max-w-screen-md px-4">
        <div className="">
          <p className="text-lg text-primary sm:text-start text-center">Potensi Desa</p>
          <div className="flex sm:flex-row flex-col sm:gap-0 gap-6 justify-between items-center mt-1.875">
            ...
            <Link
              href="/potensi"
              className="px-2.188 py-1.125 bg-primary rounded-lg text-white text-lg font-semibold hover:bg-primary-hover duration-500"
            >
              Lihat Potensi
            </Link>
          </div>
        ...
        <div className="grid grid-cols-12 pt-17 gap-x-6 gap-y-8 lg:pb-20 pb-10">
          {DocText.map((item, index) => (
            <PotensiCard key={index} icon={item.icon} title={item.title} text={item.text} />
          ))}
        </div>
        <Slider {...settings}>
